...
class FloatingPictures(pyglet.window.Window):
    ...
    def __init__(self, images_path=os.getcwd(), fullscreen=True, debug=False, speed=240.0,
                 max_picture_speed=0.25, min_picture_speed=0.0625, max_picture_width=0.4,
    ...
    def queue_image(self):
        ...
        while True:
            ...
            if not self.image_queue.full():
                ...
                width, height = self._get_scaled_dimensions(img, choice)
                ...
                self.image_queue.put([img_data, choice])
            else:
                time.sleep(0.5)

    def _get_scaled_dimensions(self, img, choice):
        if img.height > img.width:
            height = int(self.width * choice)
            width = int(np.ceil(img.width * height / img.height))
        else:
            width = int(self.width * choice)
            height = int(np.ceil(img.height * width / img.width))
...
class FloatingPicturesConfig:
    ...
    def __init__(self):
        ...
        self.config_options = [
            ...
            'description': 'Max picture speed in pixels moved per frame (default 0.25):',
            'default': 0.25,
            'type': float},
    ...
    def get_config(self):
        ...
        config_file.read(
            f'{os.path.dirname(os.path.abspath(sys.argv[0]))}/floatingpictures.ini')
        ...
        with open(f'{os.path.dirname(os.path.abspath(
                sys.argv[0]))}/floatingpictures.ini', 'w+', encoding='utf-8') as fh:
