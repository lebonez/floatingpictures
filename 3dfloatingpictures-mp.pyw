...
class FloatingPictures:
    def __init__(self, window, close_flag, images_path=os.getcwd(), debug=False,
            spin_speed=2, number_image_variations=10, image_scale=0.4, spin_frequency=60):
        ...
        self.window_dims = [self._window.width, self._window.height]
    ...
    def start_image_process(self):
        self.image_process = Process(target=queue_image, args=(self.images_path, self.close_flag,
            self.image_queue, self.window_dims, self.image_scale))
    ...
    def update_image(self, _):
        if not self.spin and not self.image_queue.empty():
...
def queue_image(images_path, close_flag, image_queue, window_dims, image_scale):
    image_paths = []
    while True:
        if close_flag.value:
            os._exit(1)
        ...
        if not image_queue.full():
            ...
            width, height = _get_scaled_dimensions(img, window_dims, image_scale)
            img = img.resize((width, height), Image.Resampling.LANCZOS).convert("RGBA")

            combined = _add_blurred_shadow(img)
...
def _get_scaled_dimensions(img, window_dims, image_scale):
    if img.height > img.width:
        height = int(window_dims[0] * image_scale)
        width = int(math.ceil(img.width * height / img.height))
    else:
        width = int(window_dims[0] * image_scale)
...
def _add_blurred_shadow(img, padding_size=2):
...
class FloatingPicturesConfig:
    ...
    def get_config(self):
        ...
        config_file.read(f'{os.path.dirname(os.path.abspath(sys.argv[0]))}/floating_pictures.ini')
        ...
        with open(f'{os.path.dirname(os.path.abspath(sys.argv[0]))}/floating_pictures.ini', 'w+', encoding='utf-8') as fh:
...
def run_screensaver(fullscreen, config):
    ...
    if fullscreen and len(screens) > 1:
        for screen in screens:
            process = Process(target=create_window, args=(screen, fullscreen, config, close_flag))
            process.start()
            processes.append(process)
        for process in processes:
            process.join()
    else:
        create_window(screens[0], fullscreen, config, close_flag)


def create_window(screen, fullscreen, config, close_flag):
    window = pyglet.window.Window(fullscreen=fullscreen, resizable=not fullscreen, screen=screen)
    floating_pictures = FloatingPictures(window, close_flag, debug=not fullscreen, **{conf['name']: conf['value'] for conf in config})
    try:
        pyglet.app.run()
    except AttributeError:
        pass
    floating_pictures.image_process.join()
